...
type ResponseType = InferResponseType<
  (typeof apiClient.api.access_requests)[":id"]["$patch"],
  201
>;
...
export const useEditRequest = (id?: string) => {
  ...
  const mutation = useMutation<ResponseType, Error, RequestType>({
    mutationFn: async (patchData) => {
      const response = await apiClient.api.access_requests[":id"]["$patch"]({
        param: { id },
        json: patchData,
      });
    ...
    onSuccess: (data) => {
      toast({
        title: "Success",
        description: `Successfully edited request ${data.id} as ${data.status}`,
      });
